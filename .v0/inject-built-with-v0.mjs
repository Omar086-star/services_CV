...
if (!layoutPath) {
  console.warn('[built-with-EDU-WEB] Could not find a Next.js root layout to patch')
  process.exit(0)
}
...
if (anchor) {
  if (url && contentWithoutExistingBadge.includes(url)) {
    ...
    console.warn('[built-with-EDU-WEB] Could not inject the built with v0 button before </body> or </html>')
  }
}
